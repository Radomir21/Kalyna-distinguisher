/*
 *
 * compile reference Kalyna (Windows MinGW):
 *   gcc -O3 -shared -o kalyna_ref.dll \
 *       kalyna.c tables.c kalyna_python_api.c kalyna_batch_api.c \
 *       -Wl,--out-implib,kalyna_ref.lib
 *
 * Linux:
 *   gcc -O3 -fPIC -shared -o libkalyna_ref.so \
 *       kalyna.c tables.c kalyna_python_api.c kalyna_batch_api.c
 */
 ...
 *    2. random key (xoshiro256++)
 *    3. random P0
 *    4. label=1: P1 = P0 + delta (mod 2^64)
 *       label=0: P1 = random
 *    5. KeyExpand
 *    6. C0 = encrypt_rounds(P0)
 *    7. C1 = encrypt_rounds(P1)
 *
 *  Output:
 *    labels:        n_samples байтів (0 або 1)
 *    ciphertexts0:  n_samples * block_bytes байтів
 *    ciphertexts1:  n_samples * block_bytes байтів
 *
 *  delta задається як 2 uint64 значення, що додаються до перших 2 слів стану.
 *  Для блоку 128 біт це покриває весь блок. Для 256/512 біт решта слів P1 = P0.
 * ========================================================================= */
API_EXPORT int kalyna_generate_dataset(
    ...
    kalyna_t* ctx = NULL;
    uint64_t* key_words = NULL;
    uint64_t pt0_words[8];   /* max block size = 512 bit = 8 words */
